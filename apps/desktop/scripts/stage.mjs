...
if (!fs.existsSync(path.join(res, 'icon.png'))) {
  const sharp = (await import('sharp')).default
  await sharp(path.join(repo, 'apps', 'web', 'public', 'favicon.svg'), { density: 1024 }).resize(512, 512).png().toFile(path.join(res, 'icon.png'))
}
